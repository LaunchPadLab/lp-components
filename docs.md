...
A full list of props supported by this component can be found [here][147]. Note that unfortunately `aria-*` props are **not** supported.

_Note: this component requires special styles in order to render correctly. To include these styles in your project, follow the directions in the main [README][148] file._
...
A group of checkboxes that can be used in a `redux-forms`-controlled form. 
Wraps the [CheckboxGroup][31] component in a [DropdownSelect][149] component, which displays the selected values as a list.
...
If `name` is used to set the text, it will be stripped of its prefixes and converted to [start case][150].

For instance: `'person.firstName'` becomes `'First Name'`

Note: When using third party form libraries (e.g., [Redux Form][151]), it's likely that setting the `required` prop will turn on the browser's automatic validation, which could cause the library to behave unexpectedly. If the browser validation behavior is causing issues, then add a `noValidate` prop to the form to [turn off][152] automatic validation. (e.g., `<form noValidate></form>`)

### Parameters

-   `name` **[String][134]** The name of the associated input
-   `id` **[String][134]** The id of the associated input (defaults to name) (optional, default `name`)
...
A component that displays a flash message generated by [redux-flash][153].
...
A component that displays multiple flash messages generated by [redux-flash][153].
...
A [react-router][154] `Route` component that requires an auth function to return `true` before it can be entered.
...
[147]: https://github.com/Hacker0x01/react-datepicker/blob/master/docs/datepicker.md

[148]: README.md#dateinput-styles

[149]: DropdownSelect

[150]: https://en.wikipedia.org/wiki/Letter_case#Stylistic_or_specialised_usage

[151]: https://redux-form.com

[152]: https://developer.mozilla.org/en-US/docs/Learn/HTML/Forms/Form_validation#Validating_forms_using_JavaScript

[153]: https://github.com/LaunchPadLab/redux-flash

[154]: https://github.com/ReactTraining/react-router
