...
## 15. `<Modal />` no longer overwrites the default modal or overlay class
Prior to v7, passing in a custom class name to the modal would override the default class, which would often strip away all of the styling. Now, that class will be _added_ to the default class.
...
```scss
.custom-modal {
  // add _only_ additional custom styles
}
```
