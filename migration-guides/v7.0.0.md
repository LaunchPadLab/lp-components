...
18. `@launchpadlab/lp-hoc` was removed as a dependency
With the introduction of hooks, `lp-hoc` is no longer necessary and will soon be archived. To accommodate this, the dependency was removed from this library. While this change did not introduce any breaking changes to components that rely on the functionality, it did remove exported utils which projects may currently rely on.
