...
1. The tag on `<Spinner />` now uses the class `spinner` in place of an id and supports additional classes
...
#### 9. The tag on `<Spinner />` now uses the classname `spinner` in place of an id

`<Spinner />` utilizes a classname and can accept additional custom classes in order to accomodate views where more than one spinners are necessary.
