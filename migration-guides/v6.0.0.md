...
1. The tag on `<Spinner />` now uses the classname `spinner` in place of an id
...
`<Spinner />` utilizes a classname `spinner` in order to accomodate views where more than one spinners are necessary.
...
// After
<div className={classnames('spinner', props.className)} />
```