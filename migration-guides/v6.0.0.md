...
1. `<CloudinaryFileInput />` and `<FileInput />` components now store an object (or array of objects) representing the file data as opposed to a string
1. `<CloudinaryFileInput />` and `<FileInput />` components no longer accept an `onLoad` prop
1. The `previewComponent` for a file input no longer receives a `file` prop and `value` is a file object (or array of file objects) as opposed to a string
...
## 6. `<CloudinaryFileInput />` and `<FileInput />` components now store an object (or array of objects) representing the file data as opposed to a string
...
<Form
  onSubmit={({ profilePhoto }) => {
    return updateProfilePhotoUrl(profilePhoto.url)
  }}
/>
```

Additionally, if you only have access to the url of the image, then you'll need to transform the data in the `initialValues` prop.
...
## 8. The `previewComponent` for a file input no longer receives a `file` prop and `value` is a file object (or array of file objects) as opposed to a string
