...
13. `<Table />` and `<SortableTable />` components now render a `<caption>` element with a `caption` prop it receives
...
`caption` is a node prop and defaults to null. <caption> html tag is only rendered if `caption` is present.
```jsx
// Before
...
// After
<Table caption="My Awesome Table">
  Table content
</Table>

// With component prop
<Table caption={<p>My <strong>Awesome </strong> Table</p>}>
  Table content
</Table>
```